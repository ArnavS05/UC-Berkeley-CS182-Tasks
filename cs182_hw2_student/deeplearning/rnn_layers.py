...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    next_h = x@Wx + prev_h@Wh + b.reshape(1, -1)
...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = (dnext_h  * (1 - next_h*next_h)) @ Wx.transpose()
    dprev_h = (dnext_h  * (1 - next_h*next_h)) @ Wh.transpose()
    dWx = x.transpose() @ (dnext_h  * (1 - next_h*next_h))
    dWh = prev_h.transpose() @ (dnext_h  * (1 - next_h*next_h))
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    ##############################################################################
    cache = []
    h = np.empty((x.shape[0], x.shape[1], Wh.shape[1]))
    prev_h = h0
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    H = int(Wx.shape[1]/4)
    a = x@Wx + prev_h@Wh + b.reshape(1, -1)
    ai = a[:, :H]
    af = a[:, H:2*H]
    ao = a[:, 2*H:3*H]
    ag = a[:, 3*H:]
    ...
    cache = (x, i, f, o, g, prev_h, prev_c, Wx, Wh, b, next_h, next_c)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    x, i, f, o, g, prev_h, prev_c, Wx, Wh, b, next_h, next_c = cache
